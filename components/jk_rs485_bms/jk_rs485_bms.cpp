...
void JkRS485Bms::decode_jk02_cell_info_(const std::vector<uint8_t> &data) {
  ...
  this->publish_state_(this->discharging_overcurrent_protection_release_time_sensor_, (float) jk_get_16bit(170 + offset));
  // 172  [198=172+26] 2                          TimeDcSCPR   Time for Discharge Short Circuit Protection Release
  this->publish_state_(this->discharging_short_circuit_protection_release_time_sensor_, (float) jk_get_16bit(172 + offset));
  // 174  [200=174+26] 2                          TimeCOCPR    Time for Charge Overcurrent Protection Release
  this->publish_state_(this->charging_overcurrent_protection_release_time_sensor_, (float) jk_get_16bit(174 + offset));   
  // 176  [202=176+26] 2                          TimeCSCPR    Time for Charge Short Circuit Protection Release
  this->publish_state_(this->charging_short_circuit_protection_release_time_sensor_, (float) jk_get_16bit(176 + offset)); 
  // 178  [204=178+26] 2                          TimeUVPR     Time for Single Cell Under Voltage Protection Release
  this->publish_state_(this->cell_undervoltage_protection_release_time_sensor_, (float) jk_get_16bit(178 + offset)); 
  // 180  [206=180+26] 2                          TimeOVPR     Time for Single Cell Over Voltage Protection Release
  this->publish_state_(this->cell_overvoltage_protection_release_time_sensor_, (float) jk_get_16bit(180 + offset)); 
  // .........
...
void JkRS485Bms::decode_jk02_settings_(const std::vector<uint8_t> &data) {
  ...
  // 54    4   0x1E 0x00 0x00 0x00    Charge OCP delay                    TIMBatCOCPDly   Charging Overcurrent
  // Protection Delay (s)
  ...
  // 58    4   0x3C 0x00 0x00 0x00    Charge OCP recovery time            TIMBatCOCPRDly  Charging Overcurrent
  ...
  // 66    4   0x2C 0x01 0x00 0x00    Discharge OCP delay                 TIMBatDcOCPDly   Discharge Overcurrent
  // Protection Delay
  ...
  // 70    4   0x3C 0x00 0x00 0x00    Discharge OCP recovery time         TIMBatDcOCPRDly  Discharge Overcurrent
  // Protection Release Delay
  ESP_LOGV(TAG, "  Discharge OCP recovery time: %f s", (float) jk_get_32bit(70));
  this->publish_state_(this->discharging_overcurrent_protection_recovery_delay_sensor_,
                       (float) jk_get_32bit(70) * 0.001f);
  // 74    4   0x3C 0x00 0x00 0x00    SCPR delay                          TIMBatSCPRDly    Short Circuit Protection
  // Recovery Delay
  ...
  // 82    4   0xBC 0x02 0x00 0x00    Charge OTP                          TMPBatCOT        Charging Over Temperature
  // Protection
  ...
  // 86    4   0x58 0x02 0x00 0x00    Charge OTP Recovery                 TMPBatCOTPR      Charging Over Temperature
  // Protection Recovery
  ...
  // 98    4   0x38 0xFF 0xFF 0xFF    Charge UTP                          TMPBatCUT        Charging Low Temperature
  // Protection
  ...
  // 102   4   0x9C 0xFF 0xFF 0xFF    Charge UTP Recovery                 TMPBatCUTPR      Charging Low Temperature
  // Protection Recovery
  ESP_LOGV(TAG, "  Charge UTP recovery: %f °C", (float) ((int32_t) jk_get_32bit(102)) * 0.1f);
  this->publish_state_(this->charging_lowtemperature_protection_recovery_sensor_,
                       (float) ((int32_t) jk_get_32bit(102)) * 0.1f);
  // 106   4   0x84 0x03 0x00 0x00    MOS OTP                             TMPMosOT         MOS Overtemperature
  // Protection
