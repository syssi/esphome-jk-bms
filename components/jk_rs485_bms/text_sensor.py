...
CONFIG_SCHEMA = JK_RS485_BMS_COMPONENT_SCHEMA.extend(
    {
        cv.Optional(CONF_OPERATION_STATUS): text_sensor.text_sensor_schema(icon=ICON_OPERATION_MODE),
        cv.Optional(CONF_ERRORS): text_sensor.text_sensor_schema(icon=ICON_ERRORS),
        cv.Optional(CONF_BATTERY_TYPE): text_sensor.text_sensor_schema(icon=ICON_BATTERY_TYPE),
        cv.Optional(CONF_PASSWORD): text_sensor.text_sensor_schema(icon=ICON_PASSWORD),
        cv.Optional(CONF_DEVICE_TYPE): text_sensor.text_sensor_schema(icon=ICON_EMPTY),
        cv.Optional(CONF_SOFTWARE_VERSION): text_sensor.text_sensor_schema(icon=ICON_EMPTY),
        cv.Optional(CONF_MANUFACTURER): text_sensor.text_sensor_schema(icon=ICON_EMPTY),
        cv.Optional(CONF_TOTAL_RUNTIME_FORMATTED): text_sensor.text_sensor_schema(icon=ICON_TIMELAPSE),
        cv.Optional(CONF_NETWORK_NODES_AVAILABLE): text_sensor.text_sensor_schema(icon=ICON_NETWORK),    
        cv.Optional(
            CONF_INFO_VENDORID
