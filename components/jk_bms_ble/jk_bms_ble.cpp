...
void JkBmsBle::decode_jk02_cell_info_(const std::vector<uint8_t> &data) {
  ...
  //   0x10 00: Cell Over Voltage
  //   0x14 00: Cell Over Voltage + Cell count is not equal to settings
  ...
  // 0xCA 0x03 0x10 0x00: Total runtime in seconds
  ...
  // 0xEC 0xE6 0x4F 0x00: Uptime 100ms
  //
...
void JkBmsBle::decode_jk04_cell_info_(const std::vector<uint8_t> &data) {
  ...
  ESP_LOGI(TAG, "Unknown1: %02X %02X %02X %02X", data[210], data[211], data[212], data[213]);

  // 214   4   0xFF 0xFF 0x00 0x00    Unknown2
  ESP_LOGI(TAG, "Unknown2: %02X %02X %02X %02X", data[214], data[215], data[216], data[217]);
  ...
  // 222   4   0x00 0x00 0x00 0x00    Unknown6
  ESP_LOGI(TAG, "Unknown6: %02X %02X %02X %02X", data[222], data[223], data[224], data[225]);
  ...
  ESP_LOGI(TAG, "Unknown9: %02X %02X %02X %02X", data[237], data[238], data[239], data[240]);
  ...
  ESP_LOGI(TAG, "Unknown10: %02X %02X %02X %02X", data[290], data[291], data[292], data[293]);
