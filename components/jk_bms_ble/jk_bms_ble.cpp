...
void JkBmsBle::gattc_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if,
  ...
  switch (event) {
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT: {
      ...
      if (chr == nullptr) {
        ESP_LOGE(TAG, "[%s] No control service found at device, not an JK BMS..?", ADDR_STR(ADDR_STR(this->parent_->address_str())));
        break;
      }
...
void JkBmsBle::update() {
  ...
  if (this->node_state != espbt::ClientState::ESTABLISHED) {
    ESP_LOGW(TAG, "[%s] Not connected", ADDR_STR(ADDR_STR(this->parent_->address_str())));
    return;
  }
...
bool JkBmsBle::write_register(uint8_t address, uint32_t value, uint8_t length) {
  ...
  if (status) {
    ESP_LOGW(TAG, "[%s] esp_ble_gattc_write_char failed, status=%d", ADDR_STR(ADDR_STR(this->parent_->address_str())), status);
  }
