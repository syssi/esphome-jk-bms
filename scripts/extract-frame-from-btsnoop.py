...
def append_values(values, frame_type : int): #beginng of a packet is "55:aa:eb:90", append every packet after that till we hit "55:aa:eb:90" again
  ...
  for index_of_value, value in enumerate(values):
      if value.startswith(start_value_with_frame_type): #55:aa:eb:90 or 55:aa:eb:90:[01,02,03]
        current_packet = value
        if index_of_value + 1 < len(values):
          for value_to_append  in values[index_of_value+1:]:
            if value_to_append.startswith(start_value) or value_to_append.startswith("aa:55"): ##55:aa:eb:90 start of the next frame or "aa:55" command response??
...
def values_to_vertical_string(data):
  ...
  if hex_values == []:
    return ""
  matrix = [[0 for _ in range(len(hex_values))] for _ in range(max(len(inner_list) for inner_list in hex_values))] 
...
parser.add_argument('--frame_type', type=int, default = 0, help='Frame Type 0 -> All, 1 -> settingsframe, 2 -> cell info frame, 3 device info frame')
