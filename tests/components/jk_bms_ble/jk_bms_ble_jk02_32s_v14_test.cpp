...
#include "common.h"
#include "frames_jk02_32s_v14.h"

namespace esphome::jk_bms_ble::testing {

TEST(JkBmsV14DeviceInfoTest, HardwareVersion) {
...
TEST(JkBmsV14DeviceInfoTest, SoftwareVersion) {
  TestableJkBmsBle bms;
...
TEST(JkBmsV14CellInfoTest, CellVoltages) {
  ...
  EXPECT_NEAR(cells[0].state, 3.175f, 0.001f);   // cell 1 (max)
  EXPECT_NEAR(cells[10].state, 2.851f, 0.001f);  // cell 11 (min)
  EXPECT_NEAR(cells[15].state, 3.174f, 0.001f);  // cell 16
}

TEST(JkBmsV14CellInfoTest, CellVoltageStats) {
  TestableJkBmsBle bms;
  bms.set_protocol_version(PROTOCOL_VERSION_JK02_32S);
  ...
  bms.decode_jk02_cell_info_(CELL_INFO_JK02_32S_V14);
  EXPECT_NEAR(min_v.state, 2.851f, 0.001f);
  EXPECT_NEAR(max_v.state, 3.175f, 0.001f);
  EXPECT_NEAR(min_cell.state, 11.0f, 0.1f);
  EXPECT_NEAR(max_cell.state, 1.0f, 0.1f);
  EXPECT_NEAR(delta.state, 0.324f, 0.001f);
}

TEST(JkBmsV14CellInfoTest, TotalVoltage) {
  TestableJkBmsBle bms;
  bms.set_protocol_version(PROTOCOL_VERSION_JK02_32S);
  sensor::Sensor total;
  bms.set_total_voltage_sensor(&total);
  bms.decode_jk02_cell_info_(CELL_INFO_JK02_32S_V14);
  EXPECT_NEAR(total.state, 49.526f, 0.001f);
}

TEST(JkBmsV14CellInfoTest, Temperatures) {
  TestableJkBmsBle bms;
  bms.set_protocol_version(PROTOCOL_VERSION_JK02_32S);
  ...
  EXPECT_NEAR(tube.state, 18.2f, 0.1f);
}

TEST(JkBmsV14CellInfoTest, StateOfCharge) {
  TestableJkBmsBle bms;
  bms.set_protocol_version(PROTOCOL_VERSION_JK02_32S);
  sensor::Sensor soc;
  bms.set_state_of_charge_sensor(&soc);
  bms.decode_jk02_cell_info_(CELL_INFO_JK02_32S_V14);
  EXPECT_FLOAT_EQ(soc.state, 50.0f);
}

TEST(JkBmsV14CellInfoTest, TotalRuntime) {
  TestableJkBmsBle bms;
  bms.set_protocol_version(PROTOCOL_VERSION_JK02_32S);
  sensor::Sensor runtime;
  bms.set_total_runtime_sensor(&runtime);
  bms.decode_jk02_cell_info_(CELL_INFO_JK02_32S_V14);
  EXPECT_FLOAT_EQ(runtime.state, 124641.0f);
}

TEST(JkBmsV14CellInfoTest, MosfetState) {
  TestableJkBmsBle bms;
  bms.set_protocol_version(PROTOCOL_VERSION_JK02_32S);
